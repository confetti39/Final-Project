...
from queue import Queue #내가 추가한 것
...
def keras_process_image(img):
   img = cv2.resize(img, (image_x, image_y)) #image_x, image_y 사이즈로 딱 사이즈 변경 해주는 것, 원소 개수 달라도 오류 안남
...
def text_mode(cam):
   ...
   while True:
      ...
      draw.text((180,50), " ", font =ImageFont.truetype('AppleGothic.ttf', 36), fill=(255, 0,0, 0))
      draw.text((30, 100), "Predicted text" + text, font = ImageFont.truetype('AppleGothic.ttf', 36), fill = (255, 255, 0, 0))
      #Quit - Q / Clear - C / Text to Speech - S
      ...
      for i, line in enumerate(word.split('\n')):
         ...
         draw.text((30, y), line, font=ImageFont.truetype('AppleGothic.ttf', 36), fill=(255, 255, 255, 0))
...
def recognize():
   cam = cv2.VideoCapture(0) # 1에서 0으로 바꿈
   if cam.read()[0]==False: 
      cam = cv2.VideoCapture(1) # 0에서 1로 바꿈
   text = ""
   word = ""
   count_same_frame = 0
   keypress = 1
...
recognize()
#    recognize = recognize()
#    return render_template('client.html', recognize = result)
